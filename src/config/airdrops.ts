...
export const PROTOCOL_CONTRACTS: { name: string; addresses: string[] }[] = [
  ...
  { name: 'Morpho', addresses: ['0xbbbbbbbbbb52cb7fbd6d5a6a5ee3d5a0c6fe6e47'] },
];
...
export const AIRDROPS_DB: AirdropDef[] = [
  ...
  },
  {
    ...
    token: 'SEA',
    chainId: 1,
    activityChains: [1, 137, 42161, 8453, 10, 56, 43114],
    category: 'pending',
  ...
  },
  {
    ...
    token: 'MASK',
    chainId: 1,
    activityChains: [1, 137, 42161, 10, 8453, 56, 59144, 534352, 324, 43114],
    category: 'pending',
  ...
  },
  {
    ...
    token: 'MORPHO',
    chainId: 1,
    activityChains: [1, 8453, 42161, 10],
    category: 'pending',
    officialUrl: 'https://morpho.org',
    estLabel: 'repartos rumorados al ecosistema',
    desc: 'Protocolo de préstamos blue-chip. El token ya existe pero la comunidad anticipa repartos a usuarios de deposits y vaults. El uso real de Morpho Blue y los balances en sus markets son las señales clave.',
    criteria: [
      { id: 'morpho-use', label: 'Interacción con Morpho Blue', hint: 'Deposita o pide prestado en markets de Morpho', weight: 3, eval: protocolFactor('Morpho', 2) },
      { id: 'defi', label: 'Uso DeFi general (Aave/Lido/DEX)', weight: 1, eval: protocolFactor('Lido', 1) },
      { id: 'eth-txs', label: 'Actividad en Ethereum (5+ txs)', weight: 1, eval: txFactor(1, 5) },
      { id: 'age', label: 'Cartera con 60+ días', weight: 1, eval: ageFactor(60) },
    ],
  },
  {
    id: 'linea-ecosystem',
    name: 'Linea · Ecosistema',
    token: 'LINEA',
    chainId: 59144,
    activityChains: [59144],
    category: 'accumulating',
    officialUrl: 'https://linea.build',
    claimUrl: 'https://claim.linea.build',
    estLabel: 'programas del ecosistema',
    desc: 'El claim principal de LINEA cerró, pero el ecosistema mantiene programas y recompensas por actividad (XP, Vault Bridge, ecosistema DeFi). La actividad continua en la red sigue puntuando para futuros repartos.',
    criteria: [
      { id: 'linea-txs', label: 'Actividad continua en Linea (10+ txs)', hint: 'Usa dApps del ecosistema Linea regularmente', weight: 2, eval: txFactor(59144, 10) },
      { id: 'linea-balance', label: 'Fondos en Linea', weight: 1, eval: balanceFactor(59144, 30) },
      { id: 'age', label: 'Cartera con 90+ días', weight: 1, eval: ageFactor(90) },
    ],
  },
  {
    ...
    estLabel: 'temporadas de ecosistema',
    desc: 'Tras el reparto inicial de SCR, Scroll mantiene temporadas de recompensas por actividad (Sessions). La actividad regular en la red y el uso de sus dApps acumulan elegibilidad.',
    criteria: [
      { id: 'scroll-txs', label: 'Actividad continua en Scroll (10+ txs)', hint: 'Usa dApps de Scroll en las temporadas activas', weight: 2, eval: txFactor(534352, 10) },
      { id: 'scroll-balance', label: 'Fondos en Scroll', weight: 1, eval: balanceFactor(534352, 30) },
      { id: 'age', label: 'Cartera con 60+ días', weight: 1, eval: ageFactor(60) },
    ],
  },
  {
    ...
    token: 'SIN TOKEN',
    chainId: 1,
    activityChains: [1, 8453],
    category: 'accumulating',
    officialUrl: 'https://megaeth.systems',
    estLabel: 'puntos por actividad (estimado comunitario)',
    desc: 'L2 de «rendimiento en tiempo real» respaldado por Vitalik y EigenLayer. Sin token aún: la comunidad puntúa la actividad en su ecosistema y los puentes desde Ethereum/Base. Actividad real y sostenida es el patrón clásico.',
    criteria: [
      { id: 'eth-txs', label: 'Actividad en Ethereum (10+ txs)', hint: 'Los puentes y la actividad mainnet puntúan', weight: 2, eval: txFactor(1, 10) },
      { id: 'base-txs', label: 'Actividad en Base (5+ txs)', weight: 1, eval: txFactor(8453, 5) },
      { id: 'eth-balance', label: 'Balance ETH relevante (≥0.03)', weight: 1, eval: (ctx) => { const b = ctx.nativeBalance(1); return { done: b >= 0.03, progress: R(b / 0.03) }; } },
      { id: 'age', label: 'Cartera con 60+ días', weight: 1, eval: ageFactor(60) },
    ],
  },
  {
